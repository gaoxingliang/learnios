  ...
  func retrieveWeatherInfo(_ location: CLLocation, completionHandler: @escaping WeatherCompletionHandler) {
    ...
    let task = session.dataTask(with: url) { (data, response, error) in
      ...
      guard let json = try? JSON(data: data) else {
        ...
        do {
            print(json) // This will print the below json.
        }
        catch{}
...
//        guard let tempDegrees = json["results"][0]["main"]["temp"].double,
//            let country = json["city"]["country"].string,
//            let city = json["city"]["name"].string,
//            let weatherCondition = json["list"][0]["weather"][0]["id"].int,
//            let iconString = json["list"][0]["weather"][0]["icon"].string else {
...
//                             URLQueryItem(name:"appid", value:appId)]
    ...
    // let url2 = "http://api.map.baidu.com/telematics/v3/weather?location=%E6%88%90%E9%83%BD&output=json&ak=3vMLUnjui3DNrgYHtmB62uSlX7hTtV5V"
    guard var components = URLComponents(string:"http://api.map.baidu.com/telematics/v3/weather") else {
        return nil
    }
    ...
    components.queryItems = [URLQueryItem(name:"location", value: "chengdu"),
                             URLQueryItem(name:"output", value:"json"),
                             URLQueryItem(name:"ak", value:"3vMLUnjui3DNrgYHtmB62uSlX7hTtV5V")]
